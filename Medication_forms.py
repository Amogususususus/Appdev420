...
class CreateSyrupForm(FlaskForm):
    ...
    Stock_Medication = IntegerField(label=('Stock of Medication:'), validators=[DataRequired(), NumberRange(min=31, max=1000, message='You are only allowed to set the Stock between%(min)d and %(max])d ')])
